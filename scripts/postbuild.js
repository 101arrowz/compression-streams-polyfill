...
for (const fn of ['index.js', 'ponyfill.js']) {
  const src = fs.readFileSync(path.resolve(libDir, fn), 'utf-8')
    .replace(/require\("\.\/ponyfill"\)/, `require("./ponyfill.js")`)
    .replace(/\/\*\* @class \*\//g, '/*#__PURE__*/')
  fs.writeFileSync(path.resolve(libDir, fn), src);
}
...
for (const fn of ['index.js', 'ponyfill.js']) {
  const src = fs.readFileSync(path.resolve(esmDir, fn), 'utf-8')
    .replace(/from '\.\/ponyfill'/, `from './ponyfill.mjs'`)
    .replace(/\/\*\* @class \*\//g, '/*#__PURE__*/');
